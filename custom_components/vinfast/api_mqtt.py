...
class MQTTManager:
    ...
    def _run_ai_advisor_wrapper(self, mode="trip", data_payload=None):
        core = self.core
        try:
            if not getattr(core, 'ai_api_key', None) or core.ai_api_key.strip() == "":
                return 
            ...
            ai_model = getattr(core, "ai_model", "") or core.options.get("ai_model", "gpt-4o-mini")
            vi = core.lang == "vi"
            ...
            core.trigger_callbacks()
            result = get_ai_advice(core.ai_base_url, core.ai_api_key, ai_model, mode, data_payload, context_data)
